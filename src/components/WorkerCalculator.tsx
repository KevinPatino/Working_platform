    ...
    <div className="animate-fade-in space-y-6">
      ...
      {startDate && endDate && (
        <div className="bg-white border-2 border-gray-100 rounded-xl p-6 text-center shadow-sm mt-6 animate-fade-in">
          <p className="text-gray-500 text-sm font-bold uppercase mb-2">Total Hours</p>
          <p className="text-5xl font-black text-blue-600">{totalHours}</p>
          <p className="text-xs text-gray-400 mt-2">
            From {new Date(startDate + 'T00:00:00').toLocaleDateString('en-US')} to {new Date(endDate + 'T23:59:59').toLocaleDateString('en-US')}
          </p>
        </div>
      )}
      ...
      {(!startDate || !endDate) && (
        <div className="py-8 text-center text-gray-400 border-2 border-dashed border-gray-200 rounded-xl mt-6">
          <p className="text-sm">Select both dates to see the result.</p>
        </div>
      )}
    </div>
  );
